...
final class URLSessionAdapterTests: XCTestCase {
    ...
    func testProperRequestExecution() throws {
        // GIVEN a GET Request
        let request = MockRequest(expectedResult: .success(model))
        
        // WHEN the request is executed on a URLSessionAdapter
        let sut = URLSessionAdapter()
        let task = try sut.execute(request: request) { _ in
            // Ignore
        }
        
        // THEN the original request should match the given request
        XCTAssertEqual(task.originalRequest?.httpMethod, request.urlRequest.httpMethod)
        XCTAssertEqual(task.originalRequest?.url, request.urlRequest.url)
    }
